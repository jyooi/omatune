...
The Device Database codec is in `packages/device-database`.
S2 golden tests read `fixtures/device/` when the private Fixture is present.
...
Host Play Data loads in `packages/core/src/play-data.ts`.
